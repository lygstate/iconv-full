...
async.parallel({
  ...
  for (let i = 0x8100; i < 0x10000; i++) { // Lead byte is 0x81 .. 0xFE
    let trail = i & 0xFF
    if (trail < 0x40 || (trail > 0x7E && trail < 0xA1) || trail > 0xFE) continue
    let lead = i >> 8
    let offset = (trail < 0x7F) ? 0x40 : 0x62
    let pointer = (lead - 0x81) * 157 + (trail - offset)
    let cpChar = data.cp950[i]
    let big5Char = data.$big5[pointer]
  ...
  for (let i = 0xA1; i <= 0xFE; i++) {
    for (let j = 0xA1; j <= 0xFE; j++) {
      const jis0208Char = data.$jis0208[(i - 0xA1) * 94 + (j - 0xA1)]
      const jis0212Char = data.$jis0212[(i - 0xA1) * 94 + (j - 0xA1)]
      eucJp[(i << 8) + j] = jis0208Char
      eucJp[(0x8F << 16) + (i << 8) + j] = jis0212Char
    }
  }
